...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        super().__init__()
        # 模型类型判定,例如pytorch、ONNX、TensorRT
        '''
        pt: PyTorch .pt 文件。
        jit: TorchScript .torchscript 文件。
        onnx: ONNX .onnx 文件。
        engine: TensorRT .engine 文件。
        tflite: TensorFlow Lite .tflite 文件。
        paddle: PaddlePaddle 模型等。
        '''
        w = str(weights[0] if isinstance(weights, list) else weights)
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, paddle, triton = self._model_type(w)
        # 半精度计算，只有当使用PyTorch、TorchScript、ONNX、TensorRT、Triton 后端支持 FP16，因此只有当这些后端被检测到时，fp16 参数才会被激活。

        fp16 &= pt or jit or onnx or engine or triton  # FP16
        # 判断是否需要转换数据维度顺序，在某些模型中图像通道为最后一个
